...
function TextInput({ label, value, onChange, placeHolder }) {
    return (
        <div  className='w-full'>
            ...
            <input type="text" className='h-12 w-full outline-1 border-2  rounded-full px-4 text-base transition-all' value={value} onChange={onChange} placeholder={placeHolder} />
        </div>
    )
}

function SelectInput({ label, data, value, onChange, firstOption }) {
    return (
        <div className='w-full'>
            <label hmlfor={label} className='p-1.5'>{label}</label>

            <select name={label} id={label} className='w-full h-12 rounded-full px-4 text-base transition-all bg-transparent border-2 outline-1 mt-2' value={value} onChange={onChange}>
